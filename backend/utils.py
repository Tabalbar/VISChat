    ...
    def calculate_metrics(file_path):
        ...
        for column in df.columns:
            ...
            if pd.api.types.is_numeric_dtype(df[column]):
                ...
                metrics['Quartiles'] = [
                    float(q) for q in df[column].quantile([0.25, 0.5, 0.75])]
                metrics['Interquartile Range (IQR)'] = float(
            ...
            metrics['Top 5 Values'] = {str(k): int(
                v) for k, v in df[column].value_counts().head(5).items()}
