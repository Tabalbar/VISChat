...
class LLM:
    ...
    def __message_builder__(self, system, user_prompt, csv_headers=None, csv_context=None, few_shot_context=None, chart_type=None, attributes=None, conversational_context=None):
    ...
    def __results_filter__(self, output):
        ...
        if matches:
            try:
                ...
                results = [re.sub(r'([\"\'].*?[\"\'])\s*<\s*(\d+)',
                                  r'\1 < \2', item).strip('\"') for item in items]
    ...
    def __results_filter_charts__(self, output):
        ...
        if matches:
            ...
            results = list(
                {result.split("->")[-1].strip() for result in results})
        return results
    ...
    def __prompt_self_reflection__(self, user_prompt, system, history):
        output, completion = self.__chat_wrapper__(messages=[{"role": "system", "content": system}] +
    ...
    """
        ...
        return self.__base_prompt_with_consistency__(
            messages=self.__message_builder__(
                system, user_prompt, few_shot_context=self.samples, attributes=attributes),
...
"""
    ...
    def prompt_select_dates(self, user_prompt, current_time):
        ...
        Only choose dates between January 1, 2024 to today, whch is {current_time}.
        """
        return self.__base_prompt_with_self_reflection__(user_prompt, system,
                                                         messages=self.__message_builder__(system, user_prompt), filter_method=self.__results_filter_dates__)
